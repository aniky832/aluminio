...
# ================= COTIZACIÓN =================
    ...
    c1, c2, c3 = st.columns(3)
    anc = c1.number_input("Ancho", 0.0, key="cot_anc")
    alt = c2.number_input("Alto", 0.0, key="cot_alt")
    ...
    if st.session_state.cotizacion and st.button("Calcular Cotización"):
        ...
        for p, barras in materiales.items():
            if p != "VIDRIO":
                ...
                total_al += subtotal
                detalle.append(f"{p}: {barras} x {precio} = {subtotal}")
