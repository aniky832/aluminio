...
def generar_pdf(pedido):
    ...
    for v in pedido:
        ...
        pdf.cell(
            190, 7,
            f"Vidrios: {vid['cant']} de {r(vid['alto'])} x {r(vid['ancho'])}",
            ln=True
        )
        ...
        for n, info in v['detalles'].items():
            pdf.cell(
                190, 7,
                f"- {info['cant']} {n}: {r(info['medida'])} cm",
                ln=True
            )
        ...
        if st.button("Calcular Optimización"):
            ...
            for p, piezas in todos.items():
                if p != "VIDRIO" and piezas:
                    ...
                    for i, b in enumerate(barras, 1):
                        st.write(
                            f"Tira {i}: {[r(x) for x in b]} "
                            f"| sobra {r(600 - sum(b))} cm"
                        )

        st.download_button(
            "PDF Ventanas",
            generar_pdf(st.session_state.pedido),
            "ventanas.pdf"
        )

        st.download_button(
            "PDF Optimización",
            generar_pdf_optimizacion(todos),
            "optimizacion.pdf"
        )
