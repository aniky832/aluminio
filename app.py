...
with st.form("mi_formulario"):
    ...
    # OPCIONES CORREGIDAS: Ahora tiene la lista [2, 3, 4]
    div = st.selectbox("Número de divisiones", options=[2, 3, 4])
    ...
    if enviar:
        if ancho > 0 and alto > 0:
            ...
            st.success("✅ Ventana agregada con éxito.")
        else:
    ...
    for i, v in enumerate(st.session_state.pedido, 1):
        with st.expander(f"Ventana #{i} - {v['medida']} ({v['div']} hojas)"):
            ...
            for p, lista in v['piezas'].items():
                todos[p].extend(lista)

    if st.button("🪚 CALCULAR PLAN DE CORTE"):
        st.header("📏 Optimización (600cm)")
        total_tiras = 0
        for perfil, piezas in todos.items():
            if piezas:
                st.subheader(f"🔹 {perfil}")
                barras = optimizar_barras(piezas)
                total_tiras += len(barras)
                for j, b in enumerate(barras, 1):
                    st.write(f"Tira {j}: {[round(x,1) for x in b]} | Sobra: {600-sum(b):.1f}cm")
        st.metric("Total Tiras de 6m", total_tiras)
